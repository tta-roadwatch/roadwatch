...
const FEATURES = [
  ...
  },
  {
    ...
    body: "한 번의 이상은 그날의 사정일 수 있습니다. 40m 격자로 나눠 서로 다른 주행에서 같은 위치가 반복 검출되는지 봅니다. 우연과 구조적 요인을 가르는 기준입니다.",
    tag: "40m 격자 · 세션 교차",
  },
  {
    step: "03",
...
const FAQ = [
  {
    ...
    a: "자율주행차는 주행 중 위치·속도·상태를 초 단위로 기록합니다. 차선이 지워졌거나 시야가 가린 구간에서는 차가 속도를 줄이거나 이상 신호를 남깁니다. 서로 다른 날 주행한 차량들이 같은 위치에서 반복 검출된다면, 그날의 사정이 아니라 도로환경 요인일 가능성이 큽니다. 그 지점을 찾아 현장점검을 권고합니다.",
  },
  {
  ...
  {
    q: "분석 결과를 믿을 수 있나요?",
    a: "판단 근거를 전부 공개합니다. 임계값을 왜 그 숫자로 정했는지, 무엇을 시도했다 버렸는지까지 저장소에 기록했습니다. 분석 파이프라인은 명령 한 번으로 전 과정이 재현되며, 22개 인수 기준을 통과해야 배포됩니다. 임계값과 검증 기준을 공개해 누구나 재현할 수 있도록 했습니다.",
  },
];
    ...
    <div className="rw-lp">
      ...
      >
        <div className="rw-lp-hero__inner rw-lp-hero__inner--enter">
          ...
          <h1 className="rw-lp-hero__title">
            자율주행차가 반복해서 이상 신호를 남긴 도로,
            <br />
            사고로 이어지기 전에 점검 후보를 찾습니다
          </h1>
          <p className="rw-lp-hero__lead">
            판교 제로시티 개방데이터 {num(s.records)}건을 분석해, 서로 다른 주행에서
            같은 위치에서 반복 검출된 구간을 도로관리자에게 알려드립니다.
          </p>
      ...
      <section id="problem" className="rw-lp-section">
        <Reveal className="rw-lp-section__head">
          <h2>기존 점검은 민원·사고 이후에 이뤄지는 경우가 많습니다</h2>
